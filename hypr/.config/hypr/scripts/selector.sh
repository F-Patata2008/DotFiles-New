...
# Find all directories, sort, clean, and pipe to Rofi.
find "$BASE_PROJECTS_DIR" -mindepth 1 -type d -print0 | \
    grep -vzP "^$(printf '%q' "$BASE_PROJECTS_DIR")\/.git($|\/)" | \
    sort -rz | \
    sed -z "s|^$(printf '%q' "$BASE_PROJECTS_DIR")/||" | \
...
if [ -n "$SELECTED_RELATIVE_PATH" ]; then
    # Construct the full path by prepending BASE_PROJECTS_DIR again.
    # Use printf '%q' on each part and then combine.
    FULL_PROJECT_PATH="$(printf '%q' "$BASE_PROJECTS_DIR")/$SELECTED_RELATIVE_PATH"
    # Execute your env.sh script with the full, safely quoted project path.
    ~/.config/hypr/scripts/env.sh "$FULL_PROJECT_PATH"
else
